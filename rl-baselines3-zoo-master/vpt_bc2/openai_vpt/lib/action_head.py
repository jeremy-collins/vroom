...
class CategoricalActionHead(ActionHead):
    ...
    def logprob(self, actions: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        ...
        print("action_head.py CategoricalActionHead value: ", value)
        value, log_pmf = torch.broadcast_tensors(value, logits)
        value = value[..., :1]
        print("action_head.py CategoricalActionHead value: ", value)
        print("action_head.py CategoricalActionHead log_pmf: ", log_pmf)

        #print("gather(-1, value).squeeze(-1): ", torch.gather(-1, value).squeeze(-1))
        result = log_pmf.gather(-1, value).squeeze(-1)

        print("action_head.py action_head.py CategoricalActionHead result1: ", result)
        ...
        print("action_head.py CategoricalActionHead result2: ", result)
        return result
...
class DictActionHead(nn.ModuleDict):
    ...
    def logprob(self, actions: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        print("action_head.py logprob DictActionHead")
        print("action_head.py actions[0], logits[0]: ")
        #print(actions[k], logits[k] for k, subhead in self.items())
        '''
        for k, subhead in self.items():
            print("k: ", k)
            print("subhead: ", subhead)
        '''
        print("actions['buttons']: ", actions["buttons"])
        print("logits['buttons']: ", logits["buttons"])
        #print("subhead: ", subhead)
