...
'''
TARGET_ACTION_SPACE = {   
    "actions": spaces.Box(low=-1.0, high=1.0, shape=(4,))
}
    # "gripper_x": spaces.Box(low=-1.0, high=1.0, shape=(4,)),
    # "gripper_y": spaces.Box(low=-1.0, high=1.0, shape=(4,)),
    # "gripper_z": spaces.Box(low=-1.0, high=1.0, shape=(4,)),
    # "gripper_width": spaces.Box(low=-1.0, high=1.0, shape=(4,)),
#}
...
class MineRLAgent:
    def __init__(self, device=None, policy_kwargs=None, pi_head_kwargs=None):
        ...
        print("agent.py MineRLAgent: self.action_mapper: ", self.action_mapper)
        
        action_space = self.action_mapper.get_action_space_update()
        print("agent.py MineRLAgent: action_space: ", type(action_space))
        print("agent.py MineRLAgent: action_space: ", action_space)
        action_space.pop('camera', None) 
        print("agent.py MineRLAgent: action_space camera removed: ", action_space)
    ...
    def _agent_action_to_env(self, agent_action):
        ...
        '''
        action = {"actions": agent_action["actions"].cpu().numpy()} ####
    ...
    def get_action(self, minerl_obs):
        ...
        call `reset()`.
        """
        agent_input = self._env_obs_to_agent(minerl_obs) # resize img
