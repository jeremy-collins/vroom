...
python3 dec_transformers.py


cd /home/cody/Documents/Project/rl-baselines3-zoo-master
tensorboard --logdir my_models/dec_transformer/
...
def flatten_image(img, low_res):
    ...
    return obs_space_flat, grayscaled_image


#load_trained_model = False
model_path = "my_models/dec_transformer/" # ./decision_transformer_gym_replay/trained_models/"
...
if delete_old_data == True:
    ...
    if os.path.exists("/home/cody/.cache/huggingface/datasets/decision_transformer_gym_replay/" + enviroment + "/1.1.0"): #/datasets/decision_transformer_gym_replay/" + env_name):
        print("exists")
        shutil.rmtree("/home/cody/.cache/huggingface/datasets/decision_transformer_gym_replay/" + enviroment + "/1.1.0")      #PandaPickAndPlace-v1/1.1.0")
...
class DecisionTransformerGymDataCollator:
    ...
    def __init__(self, dataset) -> None:
        ...
        states = observations.reshape(-1, self.state_dim) ##
        self.n_traj = observations.shape[0] ##
        traj_lens = np.ones(self.n_traj) * observations.shape[1] ###
        self.p_sample = traj_lens / np.sum(traj_lens) ###
...
training_args = TrainingArguments(
    ...
    per_device_train_batch_size=150, #64
...
if test_model == True:
    ...
    image_array = []
    combined_color_array = []
    gray_image_array = []
    num_videos = 100 # number of episode videos to produce
    ...
    for episode in range(num_videos):
        ...
        for t in range(50): #max_ep_len):
            ...
            if use_video == True:
                img = env.render("rgb_array") #rgba env.render("human") ##
                combined_color_array.append(img)
                obs_space_flat, grayscaled_image = flatten_image(img, low_res)
                gray_image_array.append(grayscaled_image)
                combined_gray_array.append(grayscaled_image)
            ...
            if done:
                if use_video == True:
                    save_video(gray_image_array, output_dir, "videos_gray/video_{}.mp4".format(episode))
                    combined_gray_array.append(gray_image_array)
                    gray_image_array = []
                env.reset()
                print("break")
                break
    ...
    if use_video == True:
        ...
        save_video(combined_gray_array, output_dir, "videos_gray/videos_combined.mp4")
